#!/usr/bin/env python3
"""
Move MTG cards without any guide sections to a 'pending_guide' collection.
...
def move_cards_to_pending(keep_count=0):
    ...
    # Find cards that have no sections at all AND have valid UUIDs
    query = {
        '$and': [
            {
                '$or': [
            ...
            {
                '$or': [
                    {'uuid': {'$exists': True, '$ne': None, '$ne': ''}},
                    {'id': {'$exists': True, '$ne': None, '$ne': ''}},
                    {'oracle_id': {'$exists': True, '$ne': None, '$ne': ''}}
                ]
            }
        ]
    }
    ...
    # Create indexes on pending_guide collection for fast lookups
    try:
        # Create a compound index that allows null values but ensures uniqueness when they exist
        pending_guide.create_index([
            ('uuid', 1),
            ('id', 1),
            ('oracle_id', 1)
    ...
    for card in cursor:
        ...
        try:
            ...
            if moved_count % 100 == 0:
                logger.info(f"Moved {moved_count:,} / {total_pending:,} cards...")
...
def show_statistics():
    ...
    # Check if pending_guide collection exists
    collection_names = db.list_collection_names()
    has_pending_guide = 'pending_guide' in collection_names
    pending_guide = db.pending_guide if has_pending_guide else None
    ...
    pending_count = pending_guide.count_documents({}) if has_pending_guide else 0
